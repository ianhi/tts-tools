...
from .generator import get_minimal_voice_name, get_all_voice_names, validate_audio_file, synthesize_raw_audio
...
class AsyncAudioGenerator:
    ...
    async def process_word_recording_async(
        ...
        async with self.tts_semaphore:
            try:
                console.print(f"[dim]Synthesizing: {word_text} ({voice_config['voice_name']})[/dim]")
                # Synthesize audio
    ...
    async def generate_all_audio_async(
        ...
        console.print(f"[bold blue]🚀 Async Processing {total_words} unique words with {len(voices)} voices[/bold blue]")
    ...
    def _print_async_summary(self, results: Dict[str, Any]) -> None:
        ...
        total_attempts = results["successful"] + results["failed"] + results["skipped"] + results["regenerated"]
        ...
        if results["failed"] > 0:
            ...
            if failed_words:
                ...
                for word in sorted(failed_words):
                    ...
                    failure_count = sum(1 for result in results["words"][word]["results"] 
                                      if result["status"] == "failed")
