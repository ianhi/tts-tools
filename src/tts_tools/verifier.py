...
from .utils import load_audio_manifest, load_minimal_pairs_data
...
class PronunciationVerifier:
    ...
    def verify_audio_file(
        self, audio_file_path: str, expected_text: str, category: str, voice_name: str
    ) -> VerificationResult:
...
def save_verification_results(results: dict[str, Any], output_path: Path) -> None:
    ...
    results_with_timestamp = {"timestamp": time.strftime("%Y-%m-%d %H:%M:%S"), **results}
